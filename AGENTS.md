...
- Before every agent-triggered normal Release build, compose a specific behavioral summary of all changes since the last commit and supply it to `build.ps1 -Message`. A manually launched workspace build must reuse an existing pending message and prompt only when none exists.
